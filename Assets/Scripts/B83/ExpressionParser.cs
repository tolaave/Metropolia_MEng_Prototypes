...
namespace B83.ExpressionParser
{
	...
	public class OperationAnd : IValue
	{
		...
		{
			get {
				return m_Values.Select(v => v.Value).All(v => v > 0) ? 1.0f : 0.0f;
			}
		}
	...
	public class OperationOr : IValue
	{
		...
		{
			get {
				return m_Values.Select(v => v.Value).Any(v => v > 0) ? 1.0f : 0.0f;
			}
		}
	...
	public class OperationEquals : IValue
	{
		private IValue[] m_Values;
		public double Value
		{
			get {
				return m_Values.Select(v => v.Value).Distinct().Count() < 2 ? 1.0f : 0.0f;
			}
		}
		public OperationEquals(params IValue[] aValues)
		{
			// collapse unnecessary nested sum operations.
			List<IValue> v = new List<IValue>(aValues.Length);
			foreach (var I in aValues)
			{
				var sum = I as OperationEquals;
				if (sum == null)
					v.Add(I);
				else
					v.AddRange(sum.m_Values);
	...
	public class OperationNotEquals : IValue
	{
		private IValue[] m_Values;
		public double Value
		{
			get {
				return m_Values.Select(v => v.Value).Distinct().Count() == m_Values.Count() ? 1.0f : 0.0f;
			}
		}
		public OperationNotEquals(params IValue[] aValues)
		{
			// collapse unnecessary nested sum operations.
			List<IValue> v = new List<IValue>(aValues.Length);
			foreach (var I in aValues)
			{
				var sum = I as OperationNotEquals;
				if (sum == null)
					v.Add(I);
				else
					v.AddRange(sum.m_Values);
	...
	public class ExpressionParser
	{
		...
		IValue Parse(string aExpression)
		{
			...
			else if (aExpression.Contains("=="))
			{
				...
				return new OperationEquals(exp.ToArray());
			}
			else if (aExpression.Contains("!="))
			{
				string[] parts = aExpression.Split(new string[]{"!="}, StringSplitOptions.RemoveEmptyEntries);
				...
				return new OperationNotEquals(exp.ToArray());
			}
			else if (aExpression.Contains("||"))
			{
				string[] parts = aExpression.Split(new string[]{"||"}, StringSplitOptions.RemoveEmptyEntries);
				...
				return new OperationOr(exp.ToArray());
			}
			else if (aExpression.Contains("&&"))
			{
				string[] parts = aExpression.Split(new string[]{"&&"}, StringSplitOptions.RemoveEmptyEntries);
				...
				return new OperationAnd(exp.ToArray());
			}
			else if (aExpression.Contains('+'))
			{
				string[] parts = aExpression.Split('+');
